...
reaches macro-F1\,=\,0.9927 and the MLP reaches macro-F1\,=\,1.0000
(5-fold CV: $0.9970 \pm 0.0027$) on the held-out test set.  Motivated by TA feedback, we extend the pipeline with
biological validation via feature importance analysis: LR weights are projected
from PCA space back to gene space, revealing well-established tissue-specific
markers (e.g., NKX2-1/TTF-1 for LUAD, PCA3 for PRAD, GATA3 for BRCA) as the
top discriminative genes.  MLP permutation importance confirms that the first
eight principal components together account for the vast majority of
classification signal, with a sharp drop-off thereafter.  These findings
confirm that the models learn biologically meaningful signals rather than
technical artefacts.
\end{abstract}
...
exclusively with a single cancer type.  The sole source of residual confusion
is GBM (168 samples, the smallest class), which at this sample size cannot
reliably form a well-separated centroid in PCA space.  The cluster purity
heatmap (Figure~\ref{fig:purity}) confirms that BRCA, COAD, KIRC, LUAD, and
PRAD each map cleanly to dedicated clusters.
...
$0.9968 \pm 0.0027$ and MLP achieves $0.9970 \pm 0.0027$ --- both
remarkably stable and statistically indistinguishable in CV.
...
for all cancer types.  Under LR, GBM is the only class below perfect
(F1\,=\,0.96); the MLP classifies all 542 test samples --- including all 25
GBM samples --- correctly in this run.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    MLP [512, 256]      & $0.9970 \pm 0.0027$ & \textbf{1.0000} & \textbf{1.0000} & \textbf{1.0000} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{MLP training dynamics.  Loss decreases smoothly; validation
           accuracy converges to $\approx 1.0$ within $\sim$20 epochs.}
...
\begin{figure}[H]
  ...
  \caption{Per-class F1 on the test set.  GBM is the only class where LR
           makes errors; the MLP classifies all classes perfectly in this run.}
  \label{fig:perclass}
\end{figure}

The MLP achieves perfect test-set classification (F1\,=\,1.000), 0.73
percentage points above LR (0.9927).  MLP training is stochastic, and this
run's initialisation led to a model that resolved all boundary cases correctly;
the CV F1 of $0.9970 \pm 0.0027$ is a more conservative estimate of expected
generalisation.  The consistent advantage over LR across runs suggests a mild
non-linear benefit at class boundaries, most likely at the BRCA--PRAD interface
visible in Figure~\ref{fig:pca_labels}.
...
\begin{table}[H]
  ...
  \begin{tabular}{llp{7cm}}
    ...
    GBM  & C21orf131, C2orf80, LOC643387, LOC285370, BTBD17
         & Top genes are uncharacterised loci or brain-specific transcripts;
...
by far the most critical ($\Delta F1 = 0.115$ and $0.086$ respectively), with
PC3--PC7 each contributing modestly.  PCs 9 and beyond are nearly irrelevant
for the MLP's predictions.  This is consistent with the PCA scatter plot
(Figure~\ref{fig:pca_labels}), where most visual separation is already visible
...
\subsection{Why Does MLP Outperform LR?}

The 0.73\,pp test-F1 advantage of the MLP over LR (and its perfect test score
in this run) indicates a mild non-linear benefit in PCA space.  The PCA scatter
plot (Figure~\ref{fig:pca_labels}) already shows clean two-dimensional
separation; the MLP's non-linearity provides a minor advantage only at the
...
discriminative signal, leaving limited room for a non-linear model to exploit.
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{MLP} [512, 256] achieves test macro-F1\,=\,1.0000 (5-fold CV:
        $0.9970 \pm 0.0027$), classifying all 542 held-out samples correctly.
        The 0.73\,pp gain over LR is consistent with a mild non-linear advantage
        at class boundaries; the CV estimate provides a more conservative
